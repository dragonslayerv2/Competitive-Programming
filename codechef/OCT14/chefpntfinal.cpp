...
int main(){
	...
	for(int i=0;i<n;i++)
		for(int j=0;j<m;j++)
			if(tempColor[i][j]=='W'){
				Q[queryNumber].push_back(make_query(i,j,LEFT));
				removeCells(tempColor,i,j,LEFT);
			}
	...
	for(int i=0;i<n;i++)
		for(int j=0;j<m;j++)
			if(tempColor[i][j]=='W'){
				Q[queryNumber].push_back(make_query(i,j,UP));
				removeCells(tempColor,i,j,UP);
			}
