...
template<class T,class MF,class UN,class L> class segmentTree
{
	public:
	...
	public:
		...
		void update(size_t left,size_t right,const value_type &value)
		{
...
struct LazyUpdate
{
	...
	int updateNode(int node,int lazy,size_t nodeLeft,size_t nodeRight)
	{
		node+=(nodeRight-nodeLeft+1)*lazy;
	}
	int updateLazy(int belowNode,int node)
	{
		belowNode+=node;
	}
};
