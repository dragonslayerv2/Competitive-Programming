...
void update(size_t qx1,size_t qx2,size_t qy1,size_t qy2,size_t nx1,size_t nx2,size_t ny1,size_t ny2, size_t nodeX, size_t nodeY,value_type value)
{
	if(lazy[nodeX][nodeY])
	{
		tree[nodeX][nodeY]=tree[nodeX][nodeY]+(lazy[nodeX][nodeY]*(nx2-nx1+1)*(ny2-ny1+1));
		if(nx1!=nx2&&ny1!=ny2)
		{
			lazy[leftChild (nodeX)][leftChild (nodeY)]+=lazy[nodeX][nodeY];
			lazy[leftChild (nodeX)][rightChild(nodeY)]+=lazy[nodeX][nodeY];
			lazy[rightChild(nodeX)][leftChild (nodeY)]+=lazy[nodeX][nodeY];
			lazy[rightChild(nodeX)][rightChild(nodeY)]+=lazy[nodeX][nodeY];
		}
		lazy[nodeX][nodeY]=0;
	}
