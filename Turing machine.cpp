/*
	...
	Date: 29/10/13 13:47
	Description: The code for turing machine.
*/
...
template<class value_type,class transition_function_type> class turingMachine
{
	private:
		...
		void change_state(iterator )  // function to change the state for each input. parse uses this by passing each character one by one.
		{
...
int main()
{
	typedef vector<map<char, pair<size_t,pair<bool,char> > transition_function; // type of the transition function. 
