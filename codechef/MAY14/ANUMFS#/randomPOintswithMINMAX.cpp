...
int OptimalPlane(pair<int,int> point)
{
	int Id1=-1;
	int Id2=-1;
	...
	for(int i=0;i<planes.size();i++)
		if(canUse[i])
		{
			double currentCost1=planes[i].c;
			double currentCost2=currentCost1;
			
			int count=countInRegion(*(points.begin()),planes[i].r);
			currentCost1/=count;
			currentCost2*=count;
			
			if(currentCost1<cost)
			{
				Id1=i;
				cost=currentCost1;
			}
			if(currentCost2<cost2)
			{
				Id2=i;
				cost2=currentCost2;
			}
...
int RandomPlane()
{
	...
	while(!canUse[id])
		id=rand()%min(size_t(20),planes.size());
	return id;
}

int getOptimalPlaneId(pair<int,int> point)
{
	if(rand()%30==0)
		return RandomPlane();
	return OptimalPlane(point);
}

pair<int,int> getRandomPoint()
{
	int rank=rand()%min(size_t(10),points.size());
	
	int k=0;
	set<pair<int,int> >::iterator i=points.begin();
	while(k<rank)
	{
		k++;
		i++;
...
int main()
{
	...
	{
		pair<int,int> point=getRandomPoint();
		int id=getOptimalPlaneId(point);
		
		bool reply=sendPlane(id,point.first,point.second);

		if(reply)
		{
			int lastSize=points.size();
			removeAreaOutofRegion(point,planes[id].r);
			canUse[id]=false;
			if(points.size()==1)
			{
				found=true;
		...
		{
			removeAreaInRegion(point,planes[id].r);
			if(points.size()==0)
				break;
		}
	}
