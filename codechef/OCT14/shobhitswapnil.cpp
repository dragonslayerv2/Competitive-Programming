		...
		for(int x=0;x<n;x++){
			for(int y=0;y<m;y++){
				if(tempColor[x][y]=='W')
				{
					...
					double lastWeightFactor   =(abs(maxX-n/2)*abs(maxX-n/2))+(abs(maxY-m/2)*abs(maxY-m/2))+1;
					double currentWeightFactor=abs(x-n/2)*abs(x-n/2)+abs(y-m/2)*abs(y-m/2)+1;
					lastWeightFactor=1/lastWeightFactor;
					currentWeightFactor=1/currentWeightFactor;
					if(rand()%3==0||(currentRemoved*currentWeightFactor>removedCells*lastWeightFactor||(currentRemoved*currentWeightFactor==removedCells*lastWeightFactor&&(rand()%2))))
					{
						direction=currentDirection;
						removedCells=currentRemoved;
		...
		for(int x=0;x<n;x++){
			for(int y=0;y<m;y++){
				if(tempColor[x][y]=='W')
				{
					...
					if(currentDirection==LEFT && abs(x-n/2)>abs(y-m/2)+10)
						currentWeightFactor*=2;
					else if(currentDirection==UP&&abs(y-m/2)>abs(x-n/2)+10)
						currentWeightFactor*=2;
					
					if(direction==LEFT && abs(maxX-n/2)>abs(maxY-m/2)+10)
						lastWeightFactor*=2;
					else if(direction==UP&&abs(maxY-m/2)>abs(maxX-n/2)+10)
						lastWeightFactor*=2;
					lastWeightFactor=1/lastWeightFactor;
					currentWeightFactor=1/currentWeightFactor;
					if(rand()%3==0||(currentRemoved*currentWeightFactor>removedCells*lastWeightFactor||(currentRemoved*currentWeightFactor==removedCells*lastWeightFactor&&(rand()%2))))
