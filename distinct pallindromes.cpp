...
int main()
{
	...
	long long pallindrome=pallindrome_count[sa[0]];
	for(int i=1;i<sa.size();i++)
	{
		int similiar=ceil((double(min(pallindrome_count[sa[i-1]],min(pallindrome_count[sa[i]],sa.lcp(sa[i],sa[i-1])-1))))/2);
		int nw=ceil((double(pallindrome_count[sa[i]]))/2);
		#ifdef PALLINDROME_DEBUG
		cout<<"AT "<<a.substr(sa[i])<<" ";
		cout<<"Pallindrome count= "<<pallindrome_count[sa[i]];
		cout<<" LCP = "<<sa.lcp(sa[i],sa[i-1]);
